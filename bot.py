...
async def handle_cut_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_drive_url(url):
            await status.edit_text("📥 Downloading from Google Drive...")
        ...
        if file_size > 45:
            ...
            drive_link = upload_to_drive(final_path, os.path.basename(final_path))

            conn = sqlite3.connect("bot.db")
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO clips (file_id, title, fmt) VALUES (?, ?, ?)",
                (drive_link, title, fmt)
            )
            ...
            await status.edit_text(
                f"✅ Clip saved to Drive! ID: *{clip_id}*\n\n"
                f"🔗 Preview: {drive_link}\n\n"
                f"Go to menu to broadcast it.",
                parse_mode="Markdown",
