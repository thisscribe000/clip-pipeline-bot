...
def admin_menu():
 keyboard = [
 [InlineKeyboardButton("Cut Clip", callback_data="cut_new")],
 [InlineKeyboardButton("Download Full File", callback_data="download_new")],
...
def user_menu():
 ...
 [InlineKeyboardButton("Prophecy Feed", callback_data="prophecy_menu")],
...
async def handle_convert_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 await status.edit_text(f" Converting to {caption.upper()}...")
...
async def process_queue(app):
 ...
 chat_id=chat_id,
 text=f" Processing job #{job_id}: *{title}*",
 parse_mode="Markdown"
 )
...
async def auto_deliver_prophecies(app):
 ...
 try:
 if aid:
 await app.bot.send_audio(chat_id=tid, audio=aid, caption=f" *{title}*", parse_mode="Markdown")
 elif vid:
 await app.bot.send_video(chat_id=tid, video=vid, caption=f" *{title}*", parse_mode="Markdown")
 elif text:
 msg = f" *{title}*\n\n{text}"
 await app.bot.send_message(chat_id=tid, text=msg, parse_mode="Markdown")
 else:
 await app.bot.send_message(chat_id=tid, text=f" *{title}*", parse_mode="Markdown")
 ...
 await app.bot.send_message(
 chat_id=ADMIN_ID,
 text=f" Prophecy delivered: *{title}* → {email}",
 parse_mode="Markdown"
 )
 except Exception as e:
 ...
 *Delivery:* {total_sent} | {total_failed} """
 ...
 " *Unsubscribe* anytime\n\n"
 "Built with for seamless clip delivery.",
 parse_mode="Markdown",
 reply_markup=user_menu()
 )
